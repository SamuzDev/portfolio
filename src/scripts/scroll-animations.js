...
function initScrollAnimations(isHashChange = false) {
  ...
  elements.forEach(el => {
    ...
    // On hash change (anchor nav), reveal immediately if in viewport
    // On initial load, let IntersectionObserver handle it for smooth animation
    if (isHashChange && isInViewport) {
      el.classList.add('is-visible');
    } else {
      observer.observe(el);
    }
  ...
  // Fallback: reveal all after 3s in case observer fails
  setTimeout(() => {
